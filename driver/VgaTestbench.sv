...
module VgaTestbench();
     ...
     */
    task assertColorOn();
        if (red_display !== 4'b1111 || green_display !== 4'b1111 || blue_display !== 4'b1111) begin
            $display("%0t ps: color expected ON (white: R=1111, G=1111, B=1111), got R=%b, G=%b, B=%b", 
                     $time, red_display, green_display, blue_display);
        end
    endtask
     ...
     */
    task assertColorOff();
        if (red_display !== 4'b0000 || green_display !== 4'b0000 || blue_display !== 4'b0000) begin
            $display("%0t ps: color expected OFF (black: R=0000, G=0000, B=0000), got R=%b, G=%b, B=%b", 
                     $time, red_display, green_display, blue_display);
        end
    endtask
